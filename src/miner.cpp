...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TrawMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TrawMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("TrawMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("traw-miner");
...
///////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running TrawMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
