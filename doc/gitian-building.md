...
*Setup instructions for a gitian build of Traw using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Traw
...
Clone the git repositories for traw and gitian and then checkout the traw version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/BitcoinGOAT/Traw.git
cd traw
...
Follow the instructions in [doc/release-process.md](release-process.md) in the traw repository
...
To build Traw (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the traw repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/traw/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/BitcoinGOAT/Traw.git
...
For example:
```bash
URL=https://github.com/crowning-/traw.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit traw=${COMMIT} --url traw=${URL} ../traw/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit traw=${COMMIT} --url traw=${URL} ../traw/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit traw=${COMMIT} --url traw=${URL} ../traw/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/traw-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/traw-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/traw-build.assert
```
...
[traw/gitian.sigs](https://github.com/traw/gitian.sigs/) repository, or if that's not possible to create a pull
request.
